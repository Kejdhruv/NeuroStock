...
const GoldSellingHistory = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:3001/Profile/GoldSold/${userId}`);
  ...
  <div key={index} className="sell-row">
    ...
    <div className="sell-name">{item.Name}</div>
    <div className="sell-symbol">{item.carrat}</div>
    <div className="sell-qty">x{item.weight}</div>
...
export default GoldSellingHistory;